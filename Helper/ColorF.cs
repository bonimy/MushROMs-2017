...
namespace Helper
{
    ...
    public struct ColorF
    {
        ...
        /// <summary>
        /// Creates a <see cref="ColorF"/> structure from a <see cref="Color"/>.
        /// </summary>
        /// <param name="color">
        /// The <see cref="Color"/> to use.
        /// </param>
        /// <returns>
        /// A <see cref="ColorF"/> structure with color components having matching intensities
        /// as those supplied in <paramref name="color"/>.
        /// </returns>
        /// <remarks>
        /// <see cref="Color"/> structures keep track of named colors and distinguish them from
        /// unnamed colors, even if they have identical color components. This is important
        /// to know when testing for color equality against named colors. The name information is
        /// completely lost in the <see cref="ColorF"/> structure. The <see cref="operator ==(Color, ColorF)"/>
        /// and all equivalent methods account for this already, by converting the <see cref="Color"/>
        /// to its ARGB value only.
        /// </remarks>
        /// <seealso cref="Color"/>
        public static ColorF FromSystemDrawingColor(Color color)
        {
            var scale = 1 / (float)Byte.MaxValue;
            var alpha = scale * color.A;
            var red = scale * color.R;
            var green = scale * color.G;
            var blue = scale * color.B;

            return new ColorF(alpha, red, green, blue);
        }
        /// <summary>
        /// Converts a <see cref="ColorF"/> structure to a <see cref="Color"/>.
        /// </summary>
        /// <param name="color">
        /// The <see cref="ColorF"/> to convert.
        /// </param>
        /// <returns>
        /// A <see cref="Color"/> structure with color components having matching intensities
        /// as those supplied in <paramref name="color"/>. The components are rounded to the
        /// nearest integer.
        /// </returns>
        /// <inheritdoc cref="FromSystemDrawingColor(Color)" select="remarks"/>
        /// <seealso cref="Color"/>
        public static Color ToSystemDrawingColor(ColorF color)
        {
            var scale = (float)Byte.MaxValue;
            var alpha = (int)((scale * color.Alpha) + 0.5f);
            var red = (int)((scale * color.Red) + 0.5f);
            var green = (int)((scale * color.Green) + 0.5f);
            var blue = (int)((scale * color.Blue) + 0.5f);
            return Color.FromArgb(alpha, red, green, blue);
        }

        /// <inheritdoc cref="FromSystemDrawingColor(Color)"/>
        public static explicit operator ColorF(Color color)
        {
            return FromSystemDrawingColor(color);
        }
        /// <inheritdoc cref="ToSystemDrawingColor(ColorF)"/>
        public static explicit operator Color(ColorF color)
        {
            return ToSystemDrawingColor(color);
        }
        ...
        /// <summary>
        /// Compares a <see cref="ColorF"/> object to a
        ...
        /// </returns>
        public static bool operator ==(ColorF left, Color right)
        {
            // Convert the system drawing color to an ARGB color since named colors do not
            // equal their unnamed counterparts.
            return (Color)left == Color.FromArgb(right.ToArgb());
        }
        /// <summary>
        /// Compares a <see cref="ColorF"/> object to a
        /// <see cref="Color"/> object. The result specifies whether
        ...
        /// <see cref="ColorF"/> object. The result specifies whether
        /// they convert to equal <see cref="Color"/> values.
        /// </summary>
        /// <param name="left">
        /// A <see cref="Color"/> to compare.
        ...
        /// converted to <see cref="Color"/>, are equal; otherwise false.
        /// </returns>
        public static bool operator ==(Color left, ColorF right)
        {
            return right == left;
        }
        /// <summary>
